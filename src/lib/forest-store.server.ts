...
export type CameraStatus = {
  ...
  activityBoost: number; // adaptive coordination
};
...
type Store = {
  ...
  recent: Map<string, number>; // dedupe key -> last ts
};
...
function seed(): Store {
  const cameras: CameraStatus[] = [
    { id: "cam_01", name: "Chitwan Buffer · North Gate", zone: "Zone A (Restricted)", restricted: true, lat: 27.53, lng: 84.45, battery: 82, online: true, lastSeen: new Date().toISOString(), totalDetections: 0, activityBoost: 0 },
    { id: "cam_02", name: "Sauraha River Bend", zone: "Zone B", restricted: false, lat: 27.58, lng: 84.49, battery: 67, online: true, lastSeen: new Date().toISOString(), totalDetections: 0, activityBoost: 0 },
    { id: "cam_03", name: "Bardia Tiger Corridor", zone: "Zone C (Restricted)", restricted: true, lat: 28.38, lng: 81.50, battery: 91, online: true, lastSeen: new Date().toISOString(), totalDetections: 0, activityBoost: 0 },
    { id: "cam_04", name: "Karnali Watch Post", zone: "Zone C", restricted: false, lat: 28.42, lng: 81.48, battery: 54, online: true, lastSeen: new Date().toISOString(), totalDetections: 0, activityBoost: 0 },
    { id: "cam_05", name: "Shuklaphanta Grasslands", zone: "Zone D", restricted: false, lat: 28.85, lng: 80.22, battery: 38, online: true, lastSeen: new Date().toISOString(), totalDetections: 0, activityBoost: 0 },
    { id: "cam_06", name: "Langtang Ridge Trail", zone: "Zone E (Restricted)", restricted: true, lat: 28.21, lng: 85.55, battery: 12, online: false, lastSeen: new Date(Date.now() - 1000 * 60 * 47).toISOString(), totalDetections: 0, activityBoost: 0 },
  ];
  ...
  const reports: CommunityReport[] = [
    ...
    { id: "r_004", userId: "u_anjali", userName: "Anjali Magar", species: "Bengal Tiger", description: "i saw a tiger it was going towards my neighbours house", location: "Chitwan · Buffer N", hasImage: true, timestamp: new Date(Date.now() - 1000 * 60 * 8).toISOString(), confidenceScore: 84, status: "likely_real", aiSuggestion: "approve", reviewStatus: "pending", pointsAwarded: 0 },
...
export function ingestCameraData(input: {
  ...
  // Filter empty / low confidence (edge AI already does, but defensive)
  const useful = input.detections.filter(d => d.confidence >= 0.55);
  if (useful.length === 0) return { ok: true, suppressed: "no_useful_detections" as const };

  // Dedupe: same camera + same primary object within 12s
  ...
  // Anomaly: human at night, or 3+ distinct species, or unusually high count
  ...
  // Adaptive coordination: boost neighbors
  if (hasHuman || anomaly) {
  ...
  if (hasHuman && cam.restricted) {
    s.alerts.unshift({
      id: rid("a"),
      level: "critical",
      type: "human_intrusion",
      cameraId: cam.id,
      zone: cam.zone,
      message: `Human intrusion detected in ${cam.zone} via ${cam.name}`,
      timestamp: ts,
      resolved: false,
    });
  } else if (anomaly) {
    s.alerts.unshift({
      ...
      cameraId: cam.id,
      zone: cam.zone,
      message: `Unusual activity at ${cam.name} · ${useful.map(d => d.object).join(", ")}`,
      timestamp: ts,
      resolved: false,
    });
  } else {
    ...
    if (["tiger", "elephant", "rhino", "leopard"].includes(top.object)) {
      s.alerts.unshift({
        ...
        cameraId: cam.id,
        zone: cam.zone,
        message: `${top.object.toUpperCase()} sighted at ${cam.name} (${Math.round(top.confidence * 100)}%)`,
        timestamp: ts,
        resolved: false,
      });
    }
...
export function verifyReport(input: {
  ...
  const s = getStore();
  const user = s.users.get(input.userId) ?? {
    id: input.userId, name: input.userName, points: 0, reportsCount: 0, trustScore: 50,
  };

  // Edge AI verification heuristic — produces confidence + suggestion only.
  // Points are NOT awarded until a forest official reviews the report.
  let score = 40;
  ...
  const status: CommunityReport["status"] =
    score >= 70 ? "likely_real" : score >= 45 ? "uncertain" : "likely_fake";
...
export function checkOfflineCameras() {
  ...
  for (const cam of s.cameras.values()) {
    ...
    if (stale && cam.online) {
      cam.online = false;
      s.alerts.unshift({
        id: rid("a"), level: "warning", type: "camera_offline",
        cameraId: cam.id, zone: cam.zone,
        message: `Camera ${cam.name} went offline`,
