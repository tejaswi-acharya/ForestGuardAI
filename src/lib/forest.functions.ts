...
export const postCameraData = createServerFn({ method: "POST" })
  .inputValidator((d: { camera_id: string; detections: Detection[]; battery: number; timestamp?: string }) => d)
...
export const getDashboardSummary = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return {
    ...
    alerts: {
      total: s.alerts.filter(a => !a.resolved).length,
      critical: s.alerts.filter(a => !a.resolved && a.level === "critical").length,
    },
    ...
    reports: {
      ...
      pending: s.reports.filter(r => r.reviewStatus === "pending").length,
      approved: s.reports.filter(r => r.reviewStatus === "approved").length,
    },
  };
});
...
export const getCommunity = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const all = s.reports;
  return {
    pending: all.filter(r => r.reviewStatus === "pending"),
    reviewed: all.filter(r => r.reviewStatus !== "pending").slice(0, 30),
